...
/// One-line footer shown at the foot of any report that surfaces clones.
///
/// This is the only breadcrumb from a failed gate to the instructions, so it is
/// printed whenever `text` output reports something, TTY or not — aggregators and
/// commit hooks capture stdout, and a footer that only appears on a terminal
/// would be missing from every place it is actually needed. Returned without a
/// trailing newline so each formatter controls its own spacing.
pub fn suppression_hint() -> &'static str {
    "Findings are not verdicts. Run `biston guide triage` for the remedy ladder, \
     including when `# biston: ignore -- <reason>` is the right answer."
}
...
mod tests {
    ...
    #[test]
    fn hint_points_at_the_triage_guide() {
        let hint = suppression_hint();
        assert!(
            hint.contains("biston guide triage"),
            "the footer is the only path from a failed gate to the guide",
        );
        assert!(hint.contains("# biston: ignore"), "hint should still show the inline comment");
        assert!(!hint.contains('\n'), "hint is a single line; callers add the newline");
        assert!(hint.is_ascii(), "the footer is piped and captured, so it stays ASCII");
    }
    ...
    #[test]
    fn ignore_file_tolerates_a_trailing_reason() {
        let source = b"# biston: ignore-file -- generated by protoc\ndef foo():\n    pass\n";
        assert!(
            file_has_ignore_file_comment(source),
            "the triage guide requires a reason, so the parser must accept one",
        );
    }
    ...
    #[test]
    fn inline_ignore_tolerates_a_trailing_reason() {
        let source_text = "def foo():\n    # biston: ignore -- hot loop, kept inlined\n    pass\n";
        let full_source = source_text.as_bytes();
        assert!(
            function_has_ignore_comment(source_text, full_source, 0),
            "the triage guide requires a reason, so the parser must accept one",
        );
    }

    #[test]
    fn preceding_line_ignore_tolerates_a_trailing_reason() {
        let full_source =
            b"# biston: ignore -- mirrors the C implementation\ndef foo():\n    pass\n";
        let fn_start = 49; // byte offset of "def"
        let source_text = "def foo():\n    pass\n";
        assert!(
            function_has_ignore_comment(source_text, full_source, fn_start),
            "the triage guide requires a reason, so the parser must accept one",
        );
    }
